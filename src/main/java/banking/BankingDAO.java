...
public class BankingDAO {
    ...
    public void bankTransferTransaction(int fromID, int toID, float amount) throws Exception {
        ...
        if (!verifyCustomers(fromID, toID))
            throw new NonExistentCustomerException("Au moins un des deux ID clients n'existent pas.");

        if (balanceForCustomer(fromID) < amount)
            throw new InsufficientBalanceException("Solde du compte débiteur insuffisant");
    ...
    public boolean verifyCustomers(int fromID, int toID) throws SQLException {

        String sql = "SELECT * FROM Account WHERE CustomerID = ?";
        try (Connection myConnection = myDataSource.getConnection();
                PreparedStatement stmt = myConnection.prepareStatement(sql)) {
            // On vérifie le 1° client
            stmt.setInt(1, fromID);
            ResultSet rs = stmt.executeQuery();
            if (!rs.next())
                return false;
            // On vérifie le 2° client
            stmt.setInt(1, toID);
